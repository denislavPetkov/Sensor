...
var _ = Describe("MemoryUsedPercentageSensor", func() {

	BeforeEach(func() {
		expectedSensorInterface = &memoryUsedPercentageSensor{
			sensorMetadata: testSensor,
			deviceId:       testDeviceId,
		}
	})

	Describe("getUsedMemoryPercentage()", func() {
		testSensor := &memoryUsedPercentageSensor{
			sensorMetadata: testSensor,
			deviceId:       testDeviceId,
		}
		Context("When getUsedMemoryPercentage(), then correct used memory in percentage is returned", func() {
			It("Should return correct used memory in percentage", func() {
				actual, err := testSensor.getUsedMemoryPercentage()
